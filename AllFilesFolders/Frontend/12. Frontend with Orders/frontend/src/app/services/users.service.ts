...
export class UsersService {
  ...
  register(register: Register): Observable<AuthenticationResponse> {
    return this.http.post<AuthenticationResponse>(`${this.usersAPIURL}authentication/register`, register);
  }
  ...
  login(email: string, password: string): Observable<AuthenticationResponse> {
    ...
    if (email === 'admin@example.com' && password === 'admin') {
      ...
      return this.http.post<AuthenticationResponse>(`${this.usersAPIURL}authentication/login`, { email, password });
    }
  }
